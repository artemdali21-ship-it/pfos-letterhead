...
<div class="header">
  ...
  <div class="company-tagline">GoodSupport · Официальное письмо</div>
</div>
<div class="date">{date}</div>
<div class="letter-body">{text_escaped}</div>
<div class="footer">
  <div></div>
  <div class="seal-area">М.П.<br>ПЕЧАТЬ<br>{entity_short}</div>
</div>
</body>
</html>"""
...
async def generate_pdf(req: LetterRequest):
    ...
    entity_short = req.entity_name.replace('ООО ', '').replace('ИП ', '')[:10]

    html_content = HTML_TEMPLATE.format(
        entity_name=html_lib.escape(req.entity_name),
        date=html_lib.escape(req.date),
